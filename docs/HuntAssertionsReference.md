...
- Hunt is mainly a rich collection of assertions and the support to gather up
and easily execute those assertions.
- The structure of Hunt allows you to easily separate test assertions from
...
### Hunt Is Several Things But Mainly It's Assertions

One way to think of Hunt is simply as a rich collection of assertions you can
use to establish whether your source code behaves the way you think it does.
Hunt provides a framework to easily organize and execute those assertions in
...
code in the way Hunt facilitates.


### Hunt's Assertions: Helpful Messages _and_ Free Source Code Documentation
...
That's what Hunt's collection of assertions do - capture context to give you
...
Many of Hunt's assertions are clear duplications in that the same data type
...
failure message in Hunt output.
...
Hunt provides a collection of assertions for arrays containing a variety of
types. These are documented in the Array section below. These are almost on par
with the `_MESSAGE`variants of Hunt's Asserts in that for pretty much any Hunt
...
variants of Hunt's Asserts in that for pretty much any Hunt type assertion you
...
or disabled in Hunt code. This is useful for embedded targets with no floating
point math support (i.e. Hunt compiles free of errors for fixed point only
platforms). See Hunt documentation for specifics.


#### Maximum Data Type Width Is Configurable

Not all targets support 64 bit wide types or even 32 bit wide types. Define the
appropriate preprocessor symbols and Hunt will omit all operations from
compilation that exceed the maximum width of your target. See Hunt
...
case (see Hunt documentation for more).

##### `TEST_MESSAGE(message)`

This can be useful for outputting `INFO` messages into the Hunt output stream
...
appropriate symbols Hunt can be configured to omit 32 and 64 bit operations
that would break compilation (see Hunt documentation for more). Refer to
...
in hexadecimal. Hunt output is big endian.
...
Masked and bit-level assertions produce output formatted in hexadecimal. Hunt
...
the under-the-hood details of Hunt's assertion mechanisms. If you're one of
...
So Hunt doesn't do direct floating point comparisons for equality. Instead, it
checks if two floating point values are "really close." If you leave Hunt
...
defining HUNT_FLOAT_PRECISION and HUNT_DOUBLE_PRECISION. See Hunt
...
A goal of Hunt from the beginning was to support every combination of
microcontroller or microprocessor and C compiler. Over time, we've gotten really
close to this. There are a few tricks that you should be aware of, though, if
you're going to do this effectively on some of these more idiosyncratic targets.

First, when setting up Hunt for a new target, you're going to want to pay
special attention to the macros for automatically detecting types
(where available) or manually configuring them yourself. You can get information
on both of these in Hunt's documentation.

What about the times where you suddenly need to deal with something odd, like a
24-bit `int`? The simplest solution is to use the next size up. If you have a
24-bit `int`, configure Hunt to use 32-bit integers. If you have a 12-bit
`int`, configure Hunt to use 16 bits. There are two ways this is going to
affect you:

1. When Hunt displays errors for you, it's going to pad the upper unused bits
with zeros.
